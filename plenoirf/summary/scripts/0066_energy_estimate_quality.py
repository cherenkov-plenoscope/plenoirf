...
for lk in ["RandomForest", "MultiLayerPerceptron"]:
    reconstructed_energy[lk] = json_utils.tree.read(
        opj(
...
for lk in reconstructed_energy:
    os.makedirs(opj(res.paths["out_dir"], lk), exist_ok=True)
    for pk in res.PARTICLES:
        ...
        valid_event_table = snt.logic.cut_and_sort_table_on_indices(
            table=event_table,
            common_indices=reconstructed_energy[lk][pk][mk]["uid"],
        )

        true_energy = valid_event_table["primary"]["energy_GeV"]
        reco_energy = align_on_idx(
            input_idx=reconstructed_energy[lk][pk][mk]["uid"],
            input_values=reconstructed_energy[lk][pk][mk][mk],
        ...
        json_utils.write(opj(res.paths["out_dir"], lk, f"{pk}.json"), cm)
        ...
        if pk == "gamma":
            ...
            fig.savefig(opj(res.paths["out_dir"], lk, f"{pk}_resolution.jpg"))
            sebplt.close(fig)
        ...
        fig.savefig(opj(res.paths["out_dir"], lk, f"{pk}.jpg"))
        sebplt.close(fig)
        ...
        ax_step = 0.8 * 1 / numE
        fig = sebplt.figure(sebplt.FIGURE_1_1)
        fig.text(s=lk, x=0.05, y=0.05, fontsize=8)
        ...
        fig.savefig(
            opj(res.paths["out_dir"], lk, f"{pk}_confusion_matrix_unc.jpg")
        )
        sebplt.close(fig)
