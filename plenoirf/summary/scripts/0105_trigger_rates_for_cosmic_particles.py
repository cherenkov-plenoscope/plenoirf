...
for zd in range(zenith_bin["num"]):
    ...
    for tk in TRIGGER_MODI:
        ...
        # gamma-ray
        # ---------
        gamma_dir = opj(res.paths["out_dir"], zk, "gamma", tk)

        _A = acceptance[zk]["gamma"][tk]["point"]["mean"]
        _A_au = acceptance[zk]["gamma"][tk]["point"]["absolute_uncertainty"]
        ...
        for ck in res.COSMIC_RAYS:
            ck_dir = opj(res.paths["out_dir"], zk, ck, tk)

            _Q = acceptance[zk][ck][tk]["diffuse"]["mean"]
            _Q_au = acceptance[zk][ck][tk]["diffuse"]["absolute_uncertainty"]

            R = np.zeros(num_trigger_thresholds)
            R_au = np.zeros(R.shape)
            dRdE = np.zeros(
                shape=(num_trigger_thresholds, fine_energy_bin["num"])
            )
            dRdE_au = np.zeros(shape=dRdE.shape)
...
for tk in TRIGGER_MODI:
    ...
    for zd in range(zenith_bin["num"]):
        ...
        for pk in res.PARTICLES:
            zk_pk_tk_dir = opj(res.paths["out_dir"], zk, pk, tk)
            with open(opj(zk_pk_tk_dir, "integral_rate.json"), "rt") as fin:
        ...
        R_tot_au = []
        for pk in res.PARTICLES:
            R_tot.append(inte[tk][zk][pk]["R"])
            R_tot_au.append(inte[tk][zk][pk]["R_au"])

        Rt, Rt_au = pru.sum(x=R_tot, x_au=R_tot_au)
        inte[tk][zk]["__total__"] = {"R": float(Rt), "R_au": float(Rt_au)}
...
for tk in TRIGGER_MODI:
    ...
    for zd in range(zenith_bin["num"]):
        ...
        Rbg, Rbg_au = pru.add(
            x=inte[tk][zk]["proton"]["R"],
            x_au=inte[tk][zk]["proton"]["R_au"],
            y=inte[tk][zk]["helium"]["R"],
            y_au=inte[tk][zk]["helium"]["R_au"],
        )

        snr, snr_au = pru.divide(
            x=inte[tk][zk]["gamma"]["R"],
            x_au=inte[tk][zk]["gamma"]["R_au"],
            y=Rbg,
            y_au=Rbg_au,
        )
        SNR[tk][zk] = {"snr_u": snr * 1e6, "snr_u_au": snr_au * 1e6}
